...
export default function LoudimInventoryPage() {
  ...
  const fetchProducts = async () => {
    try {
      setLoading(true)
      // Fetch only LOUDIM products
      const response = await api.admin.getInventoryByBrand('loudim') as { products: Product[] }
  ...
  const exportToExcel = async () => {
    try {
      toast.loading('Preparing LOUDIM inventory export...');
      
      const response = await api.admin.exportInventoryByBrand('loudim') as string;
      ...
      link.setAttribute('download', `loudim-inventory-${new Date().toISOString().split('T')[0]}.csv`);
      ...
      toast.dismiss();
      toast.success('LOUDIM inventory exported successfully');
    } catch (error) {
      console.error('Export failed:', error);
      toast.dismiss();
      toast.error('Failed to export LOUDIM inventory');
    }
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">LOUDIM Inventory</h1>
              <p className="text-gray-600 dark:text-gray-400">Manage LOUDIM brand products and stock levels</p>
            </div>
          </div>
          <div className="flex items-center space-x-3">
            ...
            <Link href="/admin/products/new?brand=loudim">
        ...
        <Card>
          <CardHeader>
            <CardTitle>LOUDIM Products ({filteredProducts.length})</CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-4">
                  {searchQuery || categoryFilter !== 'all' || stockFilter !== 'all' || statusFilter !== 'all'
                    ...
                    : 'No LOUDIM products in inventory yet'}
                </p>
                <Link href="/admin/products/new?brand=loudim">
