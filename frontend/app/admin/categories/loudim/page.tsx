...
export default function LoudimCategoriesPage() {
  ...
  const fetchCategories = async () => {
    try {
      setLoading(true)
      const response = await api.admin.getCategoriesByBrand('loudim') as { categories: Category[] }
      const categoriesList = response.categories || []
      setCategories(categoriesList)
    } catch (error) {
      console.error('Failed to fetch LOUDIM categories:', error)
      toast.error('Failed to load LOUDIM categories')
    } finally {
      setLoading(false)
    }
  }
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold text-gray-900 dark:text-white">LOUDIM Categories</h1>
              <p className="text-gray-600 dark:text-gray-400">Manage LOUDIM brand categories</p>
            </div>
          </div>
          <Dialog open={isCreateDialogOpen} onOpenChange={setIsCreateDialogOpen}>
            ...
            <DialogContent>
              <DialogHeader>
                ...
                <DialogDescription>
                  {editingCategory ? 'Update the category information' : 'Add a new category for LOUDIM brand'}
                </DialogDescription>
              </DialogHeader>
              <Form {...form}>
        ...
        <Card>
          <CardHeader>
            <CardTitle>LOUDIM Categories ({filteredCategories.length})</CardTitle>
          </CardHeader>
          <CardContent>
            {loading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-4">
                  {searchQuery ? 'Try adjusting your search' : 'No LOUDIM categories yet'}
                </p>
